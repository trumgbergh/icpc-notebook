...
inline int log2Floor(int x) { return x ? 32 - 1 - __builtin_clz(x) : -1; }
...
class SegmentTree {
    ...
    T query(int from, int to, int node) {
        ...
        pushDown(node);
        return merge(query(from, to, left(node)), query(from, to, right(node)));
    }
...
int main() {
    ...
    auto apply = [](int64_t& val, const int64_t& d) { return val += d; };
    auto merge = [](const int64_t& a, const int64_t& b) { return a + b; };
