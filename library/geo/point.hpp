...
struct Point {
    ...
    friend T orient(P a, P b, P c) { return (b - a).cross(c - a); }

    // Check if $\vec{u}$ and $\vec{v}$ are parallel
    // ($\vec{u} = c\vec{v}$) where $c \in R$)
    friend bool parallel(P u, P v) { return u.cross(v) == 0; }

    // Check if point $p$ lies on the segment $ab$
    friend bool onSegment(P a, P b, P p) {
        return orient(a, b, p) == 0 && min(a.x, b.x) <= p.x &&
               max(a.x, b.x) >= p.x && min(a.y, b.y) <= p.y &&
               max(a.y, b.y) >= p.y;
    }
